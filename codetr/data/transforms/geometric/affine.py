...
class RandomAffine:
    ...
    def __init__(
        ...
        self.translate = translate or (0.1, 0.1)
        self.scale = scale or (0.9, 1.1)
        self.shear = shear or 5.0
        self.fill = fill
    ...
    def _transform_boxes(self, boxes, w, h, angle, translate, scale, shear):
        ...
        new_x = m00 * corners_c[:, :, 0] + m01 * corners_c[:, :, 1] + center_x + translate[0]
        new_y = m10 * corners_c[:, :, 0] + m11 * corners_c[:, :, 1] + center_y + translate[1]
    ...
    def __call__(
        ...
        if isinstance(image, Tensor):
            ...
            transformed = F.affine(pil_img, angle, translate, scale, list(shear), fill=self.fill)
            image = F.to_tensor(transformed)
        else:
            image = F.affine(image, angle, translate, scale, list(shear), fill=self.fill)
        
        # Transform boxes
        if target is not None and "boxes" in target and len(target["boxes"]) > 0:
            target = target.copy()
            boxes = self._transform_boxes(target["boxes"].clone(), w, h, angle, translate, scale, shear)
            
            valid = (boxes[:, 2] > 0.01) & (boxes[:, 3] > 0.01)
            target["boxes"] = boxes[valid] if valid.any() else torch.zeros((0, 4))
            if "labels" in target:
                target["labels"] = target["labels"][valid] if valid.any() else torch.zeros((0,), dtype=torch.long)
